...
export const normalizeGithubPollResponse = (payload) => {
  ...
  if (payload?.access_token || payload?.accessToken || payload?.token || payload?.data?.access_token) {
    return {
      status: 'success',
      access_token: payload?.access_token || payload?.accessToken || payload?.token || payload?.data?.access_token,
      raw: payload,
    };
  }
  ...
  if (directStatus === 'success' || directStatus === 'authorized' || payload?.authorized === true) {
    return { status: 'success', access_token: payload?.access_token || payload?.accessToken || payload?.data?.access_token || null, raw: payload };
  }

  if (directStatus === 'authorization_pending') {
    return { status: 'authorization_pending', raw: payload };
  }

  if (directStatus === 'slow_down') {
    return { status: 'slow_down', raw: payload };
  }

  if (directStatus === 'expired_token') {
    return { status: 'expired_token', raw: payload };
  }

  if (directStatus === 'access_denied') {
    return { status: 'access_denied', raw: payload };
  }
...
export async function startGitHubDeviceAuth({
  ...
  const payload = await tryCandidateEndpoints({
    ...
    candidates: DEVICE_AUTH_CANDIDATES,
  });
...
export async function triggerGitHubRepoSetup({
  ...
  const payload = await tryCandidateEndpoints({
    ...
    candidates: REPO_SETUP_CANDIDATES,
  });
